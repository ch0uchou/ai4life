...
parser.add_argument(
    "--data", default="data", type=str, help="Dataset path", required=False
)
...
parser.add_argument(
    "--model", default=None, type=str, help="Model path", required=False
)
parser.add_argument(
    "--output", default=None, type=str, help="npy path to plot loss ", required=False
)
...
if args.output != None:
    ...
    if args.train:
        X_train_path, y_train_path, X_test_path, y_test_path = train(
            dataset_folder, LABELS, current_time
        )
    elif args.test:
        X_train_path, y_train_path, X_test_path, y_test_path = test(
            dataset_folder, LABELS, current_time
        )
    else:
    ...
    (
        ...
        X_train_path, y_train_path, X_test_path, y_test_path, n_steps, shuffle_flag=True
    )
    ...
    class LSTM(nn.Module):
        def __init__(self, input_dim, hidden_dim, output_dim, layer_num):
            ...
            self.lstm = torch.nn.LSTM(
                input_dim, hidden_dim, layer_num, batch_first=True
            )
    ...
    if args.model == None:
        ...
        for iter in range(1, n_iters + 1):
            ...
            category_tensor_val, input_sequence_val = (
                tensor_y_test.long(),
                tensor_X_test,
            )
